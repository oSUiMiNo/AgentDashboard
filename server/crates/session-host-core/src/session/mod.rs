...
impl SessionManager {
    ...
    /// 局所モードはそのまま `None` で通り、**設定画面の「この機械の使用上限」の区画に出る**
    /// （設計「この値は誰のものか」の 2026-09-13 の訂正）。
    ///
    /// **PC の一覧には出ない。** `server-core` の `account::no_agents` の doc が
    /// 「`"local"` を1台として並べたりはしない」と理由つきで禁じており、画面側にも
    /// 「`agents` が空ならローカル」という判定が2箇所ある。**行を1つ入れると、
    /// 別の PC 用の設定が実機に現れる。**
    ///
    /// # 控えた値が正本で、便は近道である
    ///
