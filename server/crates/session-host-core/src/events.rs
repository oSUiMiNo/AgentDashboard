//! セッションホストが「上へ報告する」口（セルフホスト化設計§2-3）。
//!
...
pub trait EventSink: Send + Sync + 'static {
    ...
    /// 別名がこの PC で何に解決されたか、の実測が変わった（設計§13-4）。
    ///
    /// モデルの表は **PC ごとのデータ**で、CLI の版が違えば中身も違う。変化は
    /// 「別名を初めて選んだとき」と「CLI が更新されて解決先が変わったとき」にしか
    /// 起きないので、定期送信はせずここで知らせる。
    ///
    /// **既定は何もしない。** 上へ運ぶ相手が居ない報告先（手元の配信・ローカルモード）では
    /// 表を送る先が無く、`GET /api/settings` がその場で組み立てて返すため。
    fn model_aliases_changed(&self, _aliases: serde_json::Value) {}

    /// 端末エミュレータを常駐させるか（セルフホスト化設計§7-2）。
    ///
    /// # モードの判定をここに置く理由
    ///
