...
#[test]
fn 作る消す移す口が存在しない() {
    // **構造で守る。** かつてここは `fs::write` も禁じており、検査の名前も
    // 「書き込みの口が存在しない」だった。`ファイルビュアにエディタ機能を追加` で
    // **上書きだけ**を通したので（設計§11）、`fs::write` を一覧から外してある。
    //
    // **外したのは1つだけである。** 作る・消す・移す・権限を触る道具は禁止のまま——
    // **「今回は上書きだけ」という決定を、この検査がそのまま担っている。**
    // 作成と削除は `フォルダとファイルを追加できるようにしてほしい` の担当で、
    // そちらが乗るときにここが落ちて、同じ照合を通したかを考える機会になる。
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "作る・消す・移す道具（{forbidden}）が入り込んでいる"
        );
    }
    // **書く道具は1つだけ。** 増えていないことを、ここで数える。
    //
    // **数えるのはコードだけで、説明文は除く。** 上の禁止の一覧と違って、こちらは
    // 「在ってよいが1つだけ」を見ている。説明文が API の名前を書けないと、
    // **なぜ1つだけなのかを書き残せなくなる**ので、行頭が `//` の行は落とす。
