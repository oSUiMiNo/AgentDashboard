...
/// `/branch` で会話が枝分かれし、**CLI 側のセッションIDが張り替わった**ことを示す行頭。
///
/// 続けて新しいIDを書く。**本物の `/branch` は押した席をそのまま枝にする**ので、
/// ここも同じく「同じプロセスのまま、名乗るIDだけが変わる」形で模す
/// （ブランチ設計§3-2 の待ち①が見ているのは、まさにこの張り替えである）。
///
/// **画面の出力を読ませるための印ではない。** ダッシュボードが張り替えを知る経路は
/// あくまでフックで、この行はテストが「擬似が枝になった瞬間」を掴むためだけに在る。
pub const BRANCHED_PREFIX: &str = "[fake-claude] branched: ";
