...
impl Record {
    ...
    /// （`人が打っていないものを、人の発言として出さない` 設計§14-1）。
    ///
    ...
    /// 中断マーカーが指している、**止められたアシスタントのメッセージ ID**
    /// （`interruptedMessageId`）。
    ///
    /// **有無が「読まれる前か後か」を分ける**（設計§15）。アシスタントが返し始めて
    /// いれば、その ID がここに入る。**欄ごと無ければ、まだ何も返し始めていない**
    /// ＝**読まれる前に止めた**である。
    ///
    /// **時間で判定しない。** 実測では分離しているが（持たない側は中央値2.2秒、持つ側は
    /// 15.9秒）、**機械の速さに依存する境目を画面の判定に使わない**——遅い日には
    /// 逆転する。記録が名乗っているものを使う。
    pub fn interrupted_message_id(&self) -> Option<&str> {
        self.raw.get("interruptedMessageId")?.as_str()
    }
