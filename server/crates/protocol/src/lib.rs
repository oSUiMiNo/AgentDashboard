...
/// **いまどの claude ログインで動いているか**を表す指紋。
///
/// # なぜ要るのか——ログインが変わったことは、値からは分からない
///
/// 使用上限は**同じ PC から N 本のセッションが同じ値を報告する**ので、受け口では
/// 遅れて届いたぶんを捨てて合流させている（`merge_rate_limits`）。その判定は
/// **「リセット時刻は前へ戻らない」という、1つのログインの中でだけ成り立つ約束**に
/// 寄りかかっている。
///
/// **利用者が別のアカウントへログインし直すと、この約束が破れる。** 新しい側の窓は
/// 前のログインより手前で戻ることがあり、そのとき合流は**新しい値を「遅れて届いた
/// もの」と読んで捨てる**——画面には前のアカウントの数字が残り続ける（2026-09-14 申告）。
///
/// 値そのものを見ても、切り替わったのか遅れて届いたのかは**原理的に見分けられない**。
/// だから**誰の上限かを値の外に添える**。
///
/// # ダッシュボードのアカウントとは別物
///
/// [`SessionMeta`] の周りには既に「アカウント」（ダッシュボードの利用者）と
/// 「PC」（[`AgentId`]）が居る。**これはそのどちらでもない**——`~/.claude.json` が
/// 名乗る、claude 側のログインである。
///
/// # 中身を復元できる形で持たない
///
/// 突き合わせに要るのは**「前と同じか」だけ**で、誰であるかは要らない。だから
/// 元の識別子は運ばず、**一方向に潰した hex** だけを持つ。画面にもログにも出さない。
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct ClaudeLoginFingerprint(pub String);
