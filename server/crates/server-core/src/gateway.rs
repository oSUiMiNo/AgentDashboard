...
async fn handle_report(
    ...
    match report {
        ...
        // 残量は [`AgentMessage::Status`] と同じ形で移し替えるが、**行き先が違う**。
        // `apply` の中で**記録を触らない受け口**へ振り分けられる——この値は会話が進む
        // たびに動くので、`status` と同じ道（`UPDATE` を打つ）を通すと書き込みが
        // 積み上がる（コンテキスト残量設計§4）。
        AgentMessage::ContextUsage { card_id, usage } => {
            hub.registry
                .apply(origin, ServerMessage::ContextUsage { card_id, usage })
                .await;
        }
