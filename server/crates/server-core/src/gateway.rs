...
impl SessionHostHub {
    ...
    /// 答えは一覧なら 512 KiB・中身なら 3 MiB を抱えていて、しかも複製が要るのは
...
/// 答えの種別だけ。**中身は載せない**（一覧 512 KiB・中身 3 MiB。設計§9-1）。
///
...
async fn handle_report(
    ...
    match report {
        ...
        // **受け口はまだ無い。フェーズ3 の担当である**（コンテキスト残量設計§2）。
        //
        // ここを埋めるまで、セルフホスト構成ではコンテキスト残量が画面に出ない。
        // **これは不具合ではなく、まだ作っていないだけ**である——ローカルモードは
        // 便がそのままブラウザへ届くので、先に動く。
        //
        // 埋めるときは上の [`AgentMessage::Status`] が型紙になるが、**`apply` を
        // そのまま呼ばないこと。** あちらは記録へ `UPDATE` を打つ。この値は3秒ごとに
        // 動くので、同じ道を通すと書き込みが積み上がる——**記録を触らない道**を作るのが
        // フェーズ3 の仕事である。
