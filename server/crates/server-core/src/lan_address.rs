...
//!
//! # Windows のものを起こすときは、絶対パスで指す
//!
//! **かつてここは「製品コードで唯一 `powershell.exe` を起こす場所」だったが、もう違う。**
//! `session_host_core::resources` が空きを読むために2本目を起こし、
//! `session_host_core::compact` が `schtasks.exe` を起こす。
//!
//! **新しい2つは絶対パスで直に指しており、そちらのほうが堅い**——`PATH` に Windows の道が
//! 載らない構成（`appendWindowsPath=false`）でも通るためである。ここが裸の名前のままなのは
//! 先に書かれたからで、**手本にするなら新しい2つのほう。**
//!
