...
mod tests {
    ...
    ///
    /// 既定は「行が無い」ことで表しているので、**変えた瞬間に、何も選んでいない
    /// 全員の通信量が黙って変わる**。選択肢を触るときに一緒に書き換えてしまいやすい
    /// ところなので、値そのものをここで固定する。
    #[test]
    fn 選択肢を足しても既定は動かない() {
        assert_eq!(DEFAULT_SCREEN_INTERVAL_MS, 20_000);
        assert_eq!(Intervals::default().screen_interval_ms, 20_000);
        // 足した 0.3秒 は選べる側にあるだけで、既定ではない
        assert!(check(SCREEN_INTERVAL_MS, &serde_json::json!(300)).is_ok());
    }
