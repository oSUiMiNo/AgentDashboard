...
// **無名セッションの上限（`PAST_UNNAMED_LIMIT = 20`）は 2026-09-10 に外した。**
//
// 元の理由は「画面から数えられる量に収める」で、機械の性能の話ではなかった（設計§13。
// 外したカード90枚に対して別のセッションが81本あり、絞らないと選べなくなる）。
//
// # なぜ外したか
//
// **利用者の指定である**（「呼び戻し一覧は上限無しにしよう」）。上限は**黙って落とす**
// ——「20件を超えたぶんは出していません」という表示が無いので、**利用者からは
// 見失ったのと区別が付かない**。実測では無名がちょうど20件で、21本目以降は既に
// 呼び出せない状態だった。
//
// # 代わりに何を置いたか
//
// 上限が守っていた「選べること」は、**並びと畳み方**で満たす——`past_sessions` が
// **名前付きを先に**並べ、画面が**枠（PJT）ごとに畳んで**出す。**件数で捨てるのを
// やめ、探し方のほうを用意した。**
//
// **同じ工事で入れること。** 上限だけ外して手当てを別にすると、片方だけ入った状態で
// 「160件が最終活動の順に並ぶだけの一覧」になり、前より使いにくくなる。
...
pub async fn api_sessions_past(
    ...
    // **確かめて無かったものも落とさない**（2026-09-10 に変えた）。
    //
    // かつてはここで `exists == Some(false)` を捨てていた。**捨てていた理由は
    // 横着ではなく、押させないためである**——消えたIDへの `--resume` は製品の中では
    // 「正常終了」に見えるので（設計§8-2。CLI の終了コードは 1 だが `SessionEnd` の
    // 名乗りが先に効く）、押した結果が「静かに終わったカードが1枚増える」になる。
    //
    // **しかし捨てると、利用者からは見失ったのと区別が付かない。** 探し続けることになる。
    //
    // **そこで「出すが押させない」に変えた。** 画面は `exists === false` の行を
    // **選べない項目**として出す（`SessionAdd.tsx`）。押せないので §8-2 の事故は
    // 起きず、**「戻せない」と分かる**ようになる。
    //
    // 見せ方は `DESIGN.md` §42.3 に従う——これは出来事ではなく**続いている状態**
    // なので、トーストではなく**行に添える札**で出す（`（未確認）` と同じ形）。
    Ok(Json(past))
}
