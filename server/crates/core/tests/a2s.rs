...
async fn 実体の無いものは印付きで残り_名前も残る() {
    ...
    // **実体の無いものも一覧に出す**（2026-09-10 に変えた）。かつては外していたが、
    // 外すと**利用者からは見失ったのと区別が付かない**——探し続けることになる。
    //
    // 外していた理由（押させないため。設計§8-2——消えたIDへの `--resume` は製品の中では
    // 「正常終了」に見える）は**画面が押させないことで満たす**。ここでは
    // **`exists` に「確かめて無かった」が立っていること**を見る
    let 消えた行 = past
        .iter()
        .find(|row| row.claude_session_id == 消えた)
        .expect("実体の無いものが一覧から落ちている（見失ったのと区別が付かない）");
    assert_eq!(
        消えた行.exists,
        Some(false),
        "実体が無いのに「確かめて無かった」が立っていない（画面が押させる形になる）"
    );
...
#[tokio::test]
async fn 実在しないものが混ざっても全部返り印が付く() {
    // **2026-09-10 に契約が変わった。** かつてここは「枠で絞る → 実在を確かめる →
    // 件数を切る」の順序を見ていた（逆にすると上限より少なくなる）。
    // **上限そのものを外した**ので、いま見るのは別のことである。
    //
    // | 変えたこと | なぜ |
    // |---|---|
    // | 無名20件の上限を外した | 黙って落ちるので、利用者からは見失ったのと区別が付かない |
    // | 実在しないものを落とさない | 同上。**代わりに印を付けて、画面が押させない** |
    //
    // 実在しないものを**いちばん新しい側**へ置くのは元のままにしてある——
    // かつて件数で切られていた位置なので、**上限が復活したら真っ先に消える場所**である。
    ...
    // かつての上限ちょうどまで「実在するもの」を積み、その上に「実在しないもの」を2本乗せる
    let かつての上限 = 20usize;
    let mut 在るもの = Vec::new();
    for n in 0..かつての上限 {
    ...
    // **実在するものが1本も欠けない。** 上限が復活すると、ここが真っ先に落ちる
    for 在る in &在るもの {
        assert!(
            past.iter().any(|row| row.claude_session_id == *在る),
            "実在するものが一覧から落ちている（件数で切っていないか）"
        );
    }
    // **実在しないものも出る。ただし印が付く**——出すのは「戻せない」と分かる
    // ようにするためで、押させるためではない（押させないのは画面の仕事）
    for 消えた in &消えたもの {
        let 行 = past
            .iter()
            .find(|row| row.claude_session_id == *消えた)
            .expect("実体の無いものが一覧から落ちている");
        assert_eq!(
            行.exists,
            Some(false),
            "実体が無いのに「確かめて無かった」が立っていない"
        );
    }
}
