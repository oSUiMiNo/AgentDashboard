...
//! # プロセス分離（nextest）が前提
//!
//! ここは環境変数（`AGENTDASHBOARD_PARSER_BIN` と擬似パーサの型）を書き換える。
//! **`make test-rust` は `cargo nextest` で走り、テストごとにプロセスが分かれる**ので
//! 互いに干渉しない。素の `cargo test` で走らせると同じプロセスの中でスレッドが並ぶため、
//! 最後に書いた値が全員に効く——擬似を指したいテストが本物を掴む形になる。
//!
//! ロックへ寄せないのは、`crates/core` のテスト用ロックを他クレートから見える場所へ
//! 出すことになり、**製品側の構造を検査の都合で動かす**ことになるため。
//! （既存のハーネス `common/mod.rs` の `build_full` も同じ書き方をしている）
//!
//! # なぜ擬似パーサを使うのか
//!
