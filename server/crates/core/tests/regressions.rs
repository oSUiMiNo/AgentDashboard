                ...
                if chars[j] == '$' && chars.get(j + 1) == Some(&'{') {
                    ...
                    while k < end && depth > 0 {
                        match chars[k] {
                            // **文字列の中のブレースは数えない。**
                            //
                            // 数えると `${ 見出し("}") }` で式が早く閉じ、**そこから
                            // 先の面が丸ごと崩れる**。崩れると、その行から先のテストが
                            // 静かに数から漏れる——走査器がいちばん危ない壊れ方をする。
                            //
                            // レビューで標本へ足して実際に踏んだ（5つ目の崩れ方）。
                            q @ ('\'' | '"' | '`') => {
                                k += 1;
                                while k < end {
                                    if chars[k] == '\\' {
...
describe('外側の名前は拾わない', () => {
  ...
  // ここから下は、レビューで「5つ目の崩れ方が無いか」を疑って足したもの。
  // **どれも実際に走らせて耐えることを確かめてある。**
  const 閉じ括弧 = `${見出し("}")} のあと`
  it('式の中の文字列にブレースが入っても拾う', () => {})
  const 割った = 全体 / 個数 / 2
  it('除算を正規表現と読み違えなければ拾う', () => {})
  it('エスケープした引用符 \' を含んでも拾う', () => {})
})
"#;
...
/// 変異1つ。**(名前, その行をどう壊すか, 出てほしいメッセージの断片)。**
///
/// 別名にしてあるのは clippy の `type_complexity` を通すためだが、**読む側にも
/// この3つ組が何なのかを名前で伝えられる**ので、逃げではなく素直な形になっている。
type 変異 = (&'static str, Box<dyn Fn(&mut Entry)>, &'static str);
