...
/// 起き直りの拾い上げが**両方のモードから呼ばれている**こと（縮小設計§4-1・テスト 3-7）。
///
/// # なぜサーバを起こさずに本文を読むのか
///
/// 確かめたいのは「**片方だけ実装していないか**」である。サーバモードを実際に起こして
/// 印の消え方を見る形にすると、**ローカルモードだけ実装しても緑になる**——サーバモードの
/// 経路を通らないからである。**呼び出しが2箇所あること自体**を見るしかない。
///
/// # 片方だけだと何が素通りするか
///
/// **サーバモードで起動したとき、印が永久に残る。** 次にローカルモードで起きたときに
/// その古い印を拾い、**前回の結果を今回のものとして読む**。`結果が印より古ければ…` が
/// 退けるので嘘の成功にはならないが、**毎回 `compact_failed` が出続ける**形になる。
#[test]
fn 起き直りの拾い上げは両方のモードから呼ばれる() {
    let lib = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/lib.rs");
    let text = std::fs::read_to_string(&lib).expect("lib.rs が読めること");
    let 回数 = text.matches("compact::settle_compact(").count();
    assert_eq!(
        回数, 2,
        "サーバモード（serve_server）とローカルモード（serve）の両方から呼ぶこと（設計§4-1）。\
         いまの呼び出し回数: {回数}"
    );
}
