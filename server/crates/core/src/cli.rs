...
enum Command {
    ...
    /// 解決後の場所を知る用途には使えない。
    ///
    /// **答えるのは「いま居るディレクトリの設定を解決した値」であって、走っている
    /// インスタンスの置き場所とは限らない。** 別のツリーから起動されていれば別の
    /// 場所になる。そこで断りを添えるが、**断りは stderr へ出す**——標準出力は
    /// 消す側が `head -1` で読む1行そのものなので、ここへ足すと
    /// 「パス＋断り書き」を1つのパスとして読み、**記録が消えなくなる**。
    StateDir,
