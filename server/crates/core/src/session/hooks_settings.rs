...
pub fn build_settings(program: &Path, port: u16, token: &str) -> Value {
...
pub fn write(card_id: CardId, port: u16, program: &Path) -> anyhow::Result<HookSettings> {
    ...
    let settings = build_settings(program, port, &token);
...
mod tests {
    ...
    fn settings() -> Value {
        build_settings(Path::new("/opt/agent dashboard/bin"), 8787, "とーくん")
    }
    ...
    fn 書き出したsettingsは読み直せる() {
        ...
        let written = write(card_id, 9999, Path::new("/usr/local/bin/agentdashboard"))
            .expect("書き出せること");
