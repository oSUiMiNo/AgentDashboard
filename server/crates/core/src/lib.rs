...
impl LocalServer {
    ...
    /// ブラウザで起きたことの書き出し先を差し込む（ログ設計§12）。
    ///
    /// **実体を `server-core` に置けない**ので、あちらは境界 trait だけを持ち、
    /// ここが差し込む（`server_core::client_logs` の冒頭）。差し込まれていない構成では
    /// 受け取って捨てる——口が 404 に変わると、ブラウザから「繋がらない」と
    /// 「残さない」が区別できなくなる。
