...
コマンドでも発行できる。画面をまだ作れない状況（最初の1台）ではこちら。**サーバをどちらの道で立てたか**で叩き方が変わる（[セルフホストで使う](selfhost.md)）。

道①（実行ファイルだけ）——サーバの機械で：

```
agentdashboard pair-token --account <アカウント名> --label 仕事用ノート
```

道②（docker compose）——献立表のあるディレクトリで：

```
docker compose exec dashboard agentdashboard pair-token --account <アカウント名> --label 仕事用ノート
```

どちらも**記録を直に触る**ので、サーバが待ち受けている必要は無い。

ただし道①では、**サーバと同じ利用者・同じ設定で叩くこと**。記録の置き場所は利用者の家の下に決まるので、`sudo` を付けたり別の利用者で叩いたりすると、**別の記録にトークンを書いてしまう**（発行は成功するのに、繋ぐと弾かれる）。

---
<br/>
<br/>

## 2. セッションホストを入れる（PC 側・〜1分）
...
| 一覧に出るが「接続なし」の印が付く | セッションホストが落ちている。起こし直すと繋がるが、**落ちる前のセッションは戻らない**（PTY は道連れで死んでいる） |
