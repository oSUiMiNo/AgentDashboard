...
| 版の保管庫（切り替え用に控えてある実行ファイル） | 記録の置き場所の中の `versions/` と `version-*` | **消える** |
...
### 版の保管庫は、記録の中にありますが消えます
記録の置き場所の中には、**版を切り替えるために控えてある実行ファイル**（`versions/`）と、次に起こす版を指すファイル（`version-current` ほか）も入っています。これらは `--purge` を付けなくても消えます。

同じ場所に居るのに扱いが逆なのは、**基準が「戻せるかどうか」だから**です。一覧と履歴は消したら戻せませんが、保管庫の中身は落とし直せます。残すと、版1つあたり数十MB が誰にも気づかれずに溜まり続けます。

ポインタも一緒に消します。残すと、入れ直したときに**消えた版を指したまま**「指す先が見つかりません」が出続けます。
