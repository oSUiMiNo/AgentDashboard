...
describe('面を閉じるのは ✕', () => {
  ...
  it.each(閉じる)('%s の %s に読み上げ用の名前が残っている', (file, testId) => {
    ...
    //
    // **「閉じる」ちょうどとは限らない。** 同じ帯に2つの「閉じる」が並ぶ場所では、
    // 何を閉じるのかまで言う必要がある——`FileView` の `file-close` は**列ごと**で、
    // タブの ✕ は**1枚だけ**（`サイドバーで開いたファイルを、タブで並べて切り替える`
    // 要件の完了条件5）。**見ているのは「読み上げで何のボタンか分かること」**なので、
    // 前に語が付くのは満たし方が細かくなっただけである
    const 中 = 部品(読む(file), testId, '</Button>')
    expect(中).toMatch(/aria-label="[^"]*閉じる"/)
  })
})
