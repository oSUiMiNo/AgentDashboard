...
/**
 * 線の箱の幅（px）。**見える線の長さそのもの**なので、動かすと 0.1.40 の
 * 「太くて短い」へ戻る（利用者の指定で 25→30px にした）。
 *
 * **`roam.css` の `.roam-line` と揃える。** 片方だけ動かすと `roam.test.ts` が落ちる。
 */
export const ROAM_BOX_W = 30

/**
 * 線の箱の高さ（px）。**見える線ではなく、たわみ代の量である。**
 *
 * インク（[`ROAM_INK_PX`]）を引いた残りが、中心線をたわませたり曲げたりできる幅になる。
 * **巻きで線自身を曲げるにはサジッタ 7.5px が要る**（設計§20-3）ので、
 * ±7.5px ＋ インク ±1.25px ＝ ±8.75px を容れて 18px（2026-08-28・フェーズ15）。
 *
 * **`roam.css` の `.roam-line` と揃える。** 片方だけ動かすと `roam.test.ts` が落ちる。
 */
export const ROAM_BOX_H = 18

/**
 * インク（塗る帯）の太さ（px）。箱の高さに対する割合が `clip-path` の縦の開きになる。
 *
 ...
 *
 * 線の箱は [`ROAM_BOX_W`]×[`ROAM_BOX_H`]＝30×18px。回転すると半対角は
 * √(15²+9²) ≈ 17.49px。尺取り虫の頂点で 1.35 倍になるので **23.6px** が要る。
 * 切り上げて 24（箱を 7px→18px にしたので 21 から上げた・2026-08-28）。
 *
 ...
 * 等速なので時刻は道のりで決まり、1区間 56px ÷ 32.42px/秒 ＝ **1.73秒**。
 ...
 *    [`ROAM_STOPS`] − 1 ＝ 60 で固定なので、ここを 12 へ上げると
 *    [`ROAM_ROAM`] を 47 へ下げるしかない——**「1つも動かさない」に反する**
 ...
 *
 * 50区間 × 56px ＋ 巻き 117.4px ＝ 2917.4px を 90秒で走るので **32.42px/秒**。
 */
...
/**
 * 巻きが箱のどちら側へ膨らむかを、**引かれた経路そのものから**読む。
 *
 * **`巻き()` の中の `回り` を持ち出さない。** あの値は縁までの余地と種で決まり、
 * さらに [`挟む`] で場に収めた結果が実際の点になる——**決めた向きと描かれた向きが
 * 食い違いうる**。引き直し（[`replanRoute`]）で巻きごと引き直される場合もある。
 * **描かれた点から読めば、どちらの場合も正しい。**
 *
 * 線は進行方向を向いているので、箱の中の +y は法線 `n = (−u.y, u.x)` にあたる。
 * 巻きの張り出しが `−n` の側なら箱の**上**へ膨らむ。
 *
 * **形を `data-shape` だけに紐付けると、半分の線が実際と逆へ曲がる**（要件2-1 は
 * 「巻くときに紐自身が曲がる」なので、逆へ曲がっては読めない）。
 */
export function roamCurlSide(stops: RoamStop[]): 'up' | 'down' {
  const 先頭 = stops.findIndex((s) => s.loop)
  const 末尾 = stops.reduce((acc, s, i) => (s.loop ? i : acc), -1)
  // 巻きが無い（引き直しで消えた・停留点が足りない）ときは、どちらでも同じなので既定を返す
  if (先頭 < 1 || 末尾 <= 先頭) return 'up'
  const A = stops[先頭 - 1]
  const B = stops[末尾]
  const 中 = stops[Math.floor((先頭 + 末尾) / 2)]
  const 伸び = Math.hypot(B.x - A.x, B.y - A.y)
  if (伸び === 0) return 'up'
  const u = { x: (B.x - A.x) / 伸び, y: (B.y - A.y) / 伸び }
  const n = { x: -u.y, y: u.x }
  const 横 = (中.x - A.x) * n.x + (中.y - A.y) * n.y
  return 横 < 0 ? 'up' : 'down'
}
