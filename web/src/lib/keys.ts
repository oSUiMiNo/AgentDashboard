 ...
 * で実測）。ここへ `╌` を入れると、選択待ちの画面が「打てる」に化ける。
 *
 ...
 * 字下げだけを見ると、この画面が「打てる」に化ける。打てる画面の側にも、入力欄とは
 * 別に**字下げ0の `❯` がもう1行**ある（どれも過去の発言）。
 *
 * **罫線に挟まれているかまで見ると、生きている入力欄とエコーが分かれる**（実物15枚で確認）。
 */
...
/**
 * いま端末が**文字を打てる状態**か。スマホでソフトキーボードを出してよいかの判定。
 *
 * ```
 * 打てる ＝ 入力欄が見えている ‖ 選択待ちに見えない
 * ```
 *
 * # 倒し方は緩く——迷ったら出す
 *
 * 3つ目の判定なので、[`isSelectionPrompt`]（厳しく）・[`looksSelecting`]（緩く）と
 * 並べて読むこと。**外したときの損の向きが、どちらとも違う。**
 *
 * | 外し方 | 何が起きるか |
 * |---|---|
 * | 偽陽性（塞ぐべき場面で出る） | **いまの振る舞いそのもの。悪化しない**。画面の外を1回タップすれば消える |
 * | 偽陰性（出すべき場面で塞ぐ） | **打つ手段が画面から消える。逃げ道が無い** |
 *
 * **「もう一度タップすれば直る」は成り立たない。** 判定は画面から決まるので、同じ画面を
 * もう一度タップすれば同じ答えが返る。直るのは遷移の途中のフレームを踏んだときだけで、
 * **知らない形の入力欄を取りこぼした場合は何度タップしても打てない。**
 *
 * 直っていない不便を残すことと、新しく打てなくすることは釣り合わないので、**緩い側へ倒す。**
 *
 * # 左が右を上書きする
 *
 * 選択肢から「その他」を選んだ直後のように、**メニューの残骸と自由入力欄が同じ画面に
 * 同居する**場面がありうる。入力欄が見えているなら、選択待ちに見えても打てる側へ倒す。
 */
export function acceptsTyping(screen: string): boolean {
  const lines = screenLines(screen)
  if (lines.length === 0) {
    return false
  }
  return hasInputBox(lines) || !looksSelecting(screen)
}
