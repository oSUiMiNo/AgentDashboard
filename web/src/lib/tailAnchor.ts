 ...
 *
 * **ただし「止めたあとの戻す」は別物である**（2026-09-08 の追補）。抑制が効いている間、
 * 位置を書き換えてくる相手は**1つも居ない**——殴り合いにならないので、戻す側が必ず勝つ。
 * 畳むときに [`planBodyToggle`] が使うのはこちらで、**止める代わりではなく、止めた上に
 * 乗せている**。
 *
 * # なぜ状態から算出するのか
 *
...
/** 開け閉めのときに、位置を控えるか・戻すか。 */
export interface BodyTogglePlan {
  /** 開くとき、この位置を控える。畳むときは `null`（控えるものが無い） */
  覚える: number | null
  /** 畳むとき、この位置へ戻す。控えが無ければ `null`（どこへも動かさない） */
  戻す: number | null
}

/**
 * 本文を開け閉めしたとき、読んでいる位置をどう扱うか。
 *
 * # なぜ戻すのか
 *
 * **開くときに跳ねなくなったので、畳むときの迷子が見えるようになった。**
 * 開いた本文は画面より長いことが多く、読み進めると**押した行はとうに画面の上**に
 * 居る。そこで畳むと、本文が消えたぶん**まわりの行が下から迫り上がってくる**——
 * 位置（`scrollTop`）は変わっていないのに、**さっきまで読んでいた行がどこにも無い**。
 *
 * **押した行の近くへ戻すのが正しい。** 畳むのは「この本文はもういい」という合図なので、
 * **押す前に居た場所**へ返す。
 *
 * # なぜ「開いた時点の位置」なのか
 *
 * 畳んだ瞬間の位置ではない。あれは**開いた本文の中のどこか**なので、畳んだ後の
 * 文書には対応する場所が無い。控えるのは**開く前＝畳んである状態で測った位置**で、
 * これは畳んだ後の文書でも**必ず届く範囲にある**（それより文書が短くなることはない）。
 *
 * # 控えは行ごとに持つ
 *
 * 別の行を開いてから畳むこともあるので、**1つしか覚えられない作りにしない。**
 * 戻したら消す——**同じ行をもう一度開けば、そのときの位置を新しく控える。**
 *
 * @param 開いていた 押す**前**に開いていたか（押した後の状態ではない）
 * @param いまの位置 押した瞬間の `scrollTop`
 * @param 覚えていた この行について控えてあった位置（無ければ `undefined`）
 */
export function planBodyToggle(
  開いていた: boolean,
  いまの位置: number,
  覚えていた: number | undefined,
): BodyTogglePlan {
  return 開いていた
    ? { 覚える: null, 戻す: 覚えていた ?? null }
    : { 覚える: いまの位置, 戻す: null }
}
