      ...
      */
      ...
      /*
        **マウスは主ボタンでしか掴まない。**
        ...
        ここは長いあいだ `button` を1度も見ていなかった。つまり**中ボタンでも右ボタンでも
        掴みの候補になり、そのまま動かせば並べ替えが始まっていた**——中クリックに
        「新しいタブで開く」を割り当てるにあたって、まずここを閉じる。

        **右ボタンも同時に外れる。** 右ドラッグで並べ替えが始まるのは誰も頼んでいない
        （メニューを出そうとして並びが変わる）。

        **指とペンは今までどおり。** あちらの `button` は 0 で来るが、条件をマウスに
        限っておかないと、`button` を持たない合成イベントで掴めなくなる端末が出る。

        **戻るのは、上の「前の運びの印」を降ろした後ろ**でなければならない。手前に置くと、
        指で運んだあと（`click` が飛ばず印が残っている状態）に中ボタンや右ボタンを押した
        だけで印が残り続け、**次の左クリックが1回食われる**——押しても何も起きないので、
        壊れているのと見分けが付かない。

        隣のイシュー `PJT画面の横スクロールをどこからでも効かせる` は**中ドラッグで
        横へ送りたい**ので、中ボタンが並べ替えから外れているほうが都合がよい。
      */
