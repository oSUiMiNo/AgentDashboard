...
export interface FileContent {
   ...
   *
   * **決めるのはサーバである。** 効いている根は「設定の `writable_roots` ＋ その口座と
   * PC で引けるプロジェクト全部」で、**後者は記録を引かないと分からない**。だから画面が
   * 自分で判定することはできないし、してはいけない——**判定する場所が2つあると食い違う。**
   *
   * 付いていないときは **`false`（書けない）に倒れる。** 書けない相手に打てる姿を
   * 見せるより、書けるのに読むだけの姿を見せるほうが害が小さい。
   */
  writable?: boolean
}
