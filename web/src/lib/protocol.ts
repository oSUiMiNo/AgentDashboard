...
export interface PastSession {
   ...
   * |---|---|---|
   * | `true` | 確かめて在った | 押せる |
   * | `null` | **確かめていない**（PC が繋がっていない・版が古い・時間切れ） | **押せる** |
   * | `false` | **確かめて無かった**（履歴が消えている） | **押せない** |
   *
   * **`null` を `false` と混同しないこと。** 前者は「聞けなかっただけで在るかも
   * しれない」なので選べる形で残す（外出先から復旧するのがこの機能の目的である）。
   *
   * # `false` が来るようになった（2026-09-10）
   *
   * かつてはサーバが一覧から外していた。**外していた理由は押させないためである**
   * ——消えたIDへの `--resume` は製品の中では「正常終了」に見えるので（設計§8-2）、
   * 押した結果が「静かに終わったカードが1枚増える」になる。
   *
   * **しかし外すと、利用者からは見失ったのと区別が付かない。** そこで
   * **「出すが押させない」**に変えた。**選べない項目として描くこと。**
   */
  exists: boolean | null
}
...
export type Node =
  | {
      ...
      /**
       * **読まれる前に取り消されたか**（設計§15）。送ったが、アシスタントが返し始める
   ...
   * が立つ（設計§14）。**字面で見分けない**——`No response requested.` のような
   ...
   * （作業中に送った追加メッセージ 設計§3-1）。
   *
   * **これは「まだ読まれていない利用者の発言」として出す**（設計§7-1・2026-09-05）。
   * 器は `user_message` と同じ右寄せの吹き出しで、畳み方もしきい値も同じものを通す。
   * **待ちであることは、地の色を青から灰色側へ寄せることだけで言う**（§7-2）。
   *
   * **記録は誰が書いたかを名乗っていない**（§1-2）ので、厳密には「人が打ったもの」
   * とは限らない。それでも同じ器へ入れるのは、**読まれれば同じ文が `user` として
   * 出てくる**ためで、器を変えると読まれた瞬間に別物が現れたように見える。
   * 出どころの名乗りは姉妹イシューの持ち分で、面が重なることは設計§7-8 にある。
   *
   * **並びの上でも特別扱いする**（§7-5）。待っているあいだは**常にいちばん下**へ
   * まとめて置き、あとから来たエージェントの発言はその上へ割り込む。
   *
   * `taken` が立ったら**行にしない**（設計§4）。単一ノードを消す経路がどこにも
   * 無いので、消す代わりに畳む。**そのとき最下部固定からも同時に外れる**ので、
   * 「読まれたら自然な位置へ戻る」は書かなくても成り立つ。
   */
  | { kind: 'queued_message'; text: string; taken: boolean }
...
/**
 * **危険度の軸に乗らないモード**の色（細かい修正 設計§3-1）。
 *
 * `permissionModeTone` は長らく危険度（low / medium / high）だけで分岐していたので、
 * **プランは手動確認とまったく同じ無彩色**を返していた。これは「プランの色を決め忘れた」
 * のではなく、**決め方の構造が足りていなかった**——プランは危険度が低いのではなく、
 * **計画を立てるだけで何も実行しない**ので「どれくらい危ないか」で位置が決まらない。
 *
 * # なぜ `blue-400` なのか（実測 2026-09-04）
 *
 * OKLCH で測った色相の隔たりは次のとおり。**設計§3-3 は紫と約45°を見込んでいたが、
 * 実測では届かなかった**ので、帯（`blue-400`〜`500`）のうち**離れているほう**を採る。
 *
 * | 相手 | `blue-400` | `blue-500` |
 * |---|---|---|
 * | 一覧の選択中の紫（`--select`） | **38.4°** | 33.2° |
 * | Primary Accent のシアン | 51.2° | 56.4° |
 *
 * **色相だけで見分けているのではない。** 紫は L 0.541 / C 0.247、`blue-400` は
 * L 0.714 / C 0.143 で、**明るさも鮮やかさも違う**——色相差が見込みに届かなくても、
 * 3つの軸が同時にずれていれば分かれる。
 *
 * **青灰（起動中・スリープの neutral 群）とは色相が 2°しか離れていない。** これは設計が
 * 挙げていなかった隣人だが、**彩度が 8倍違う**（0.023 対 0.143）ので色相ではなく
 * 鮮やかさで分かれる。**この2つを「色相が近いから」と動かさないこと**——動かすと、
 * 今度は紫かシアンへ寄る。
 *
 * **Claude Code のティール `#48968C` は採らない**（設計§3-3）。Primary Accent と
 ...
 * 何も目立たなくなる。
 *
 * **決め方は2段**（細かい修正 設計§3-1）。**専用の色を持つモードが先**で、無ければ
 * 危険度で決める——危険度の軸に乗らないものを、無理に low へ押し込めない。
 */
