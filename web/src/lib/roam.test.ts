  ...
  it('引き直しても線が飛ばない＝いまの点から続く', () => {
    ...
    */
    ...
    /*
      ...
      **ちょうど 180° は起こりうる。** 場の縁で通路が尽きると経路が真逆へ折り返す
      （実測：`(79.3, 54.0) → (24.0, 54.0)` で `-180° → -360°`）。**そこは `畳む` が
      選べる2つの等価な角が等距離になる点**なので、浮動小数の丸めが `180` を
      わずかに超える（2.8e-14）。**そこで落とすのは、この検査が守りたいものではない**
      ——守っているのは**引き継ぎを忘れた数百度の飛び**である。
      箱を高くして余白を 21→24px にしたとき、経路が縁へ当たるようになって踏んだ
      （2026-08-28・フェーズ15）。
    */
    // 丸めのぶんだけ緩める。**桁が2つ違えば、数百度の飛びは1つも見逃さない**
    ...
    for (const 種 of [1, 2, 3, 5, 17, 42, 99]) {
      ...
      for (const i of [12, 25, 40, 50]) {
        ...
        for (let k = 0; k < 並び2.length - 1; k += 1) {
          expect(Math.abs(並び2[k + 1].r - 並び2[k].r)).toBeLessThanOrEqual(折れの上限)
        }
      }
    }
  })
