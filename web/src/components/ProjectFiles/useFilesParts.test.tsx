...
describe('器が1つであること', () => {
  ...
  */
   ...
   * レールの目印から後ろを切り出す。
   *
   * **並び順そのものでは見られない。** セッション専用画面のレールは**ファイルの
   * 後ろで定義したローカル部品**で、`column` は**属性として渡してから**中で描かれる
   * ——素朴に `indexOf('{column}')` を比べると、属性のほうを拾って落ちる（実際に踏んだ）。
   *
   * **「レールの目印より後ろに何が在るか」を見れば、どちらの形でも同じ主張になる。**
   */
  const レールの中 = (中身: string, 名前: string) => {
    const 目印 = レールの目印[名前 as keyof typeof レールの目印]
    const 位置 = 中身.indexOf(目印)
    expect(位置, `${名前} にレールが在ること`).toBeGreaterThan(-1)
    return 中身.slice(位置)
  }

  it('中身の列は、どちらの画面でもレールの中に置いてある', () => {
    for (const [名前, 中身] of Object.entries(画面)) {
      expect(
        レールの中(中身, 名前),
        `${名前} の列がレールの中で描かれていること`,
      ).toContain('{column}')
    }
  })
