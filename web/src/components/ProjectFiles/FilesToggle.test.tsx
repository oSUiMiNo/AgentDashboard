...
/**
 * 大きくした（要件8・細かい修正 設計§2-3・§8-1）。**一度 1.5倍（48px）にしたが、
 * 実物の帯では隣の ✕（32px）に対して主張が強すぎたので 40px へ戻した**
 * （2026-09-06・利用者の指定「1.3倍くらいで良かった」）。
 *
...
it("器は 40px の段を使う（個別に上書きしていない）", () => {
  ...
  // `size-10` = 40px。**32px の `size-8` に戻ると落ちる**
...
it("絵も一緒に大きくなる（20px）", () => {
  ...
  /*
    **線の太さの根拠は、ここで作り直した。** 32px の器に 16px で描いていたころは
    実効 1.3px 前後で「本文の太さと揃う」と言えたが、40px に 20px で描くと実効 1.7px
    前後になる。**器・絵・線がそろって同じ倍率で伸びる**のが要件の言う「大きくする」である。
  */
  expect(button.className).toContain("[&_svg:not([class*='size-'])]:size-5");
