 ...
 * （§14.3 の Expand 系）。
 *
 * **線の太さの根拠は、大きさを変えた時点で作り直した**（細かい修正 要件8）。32px の器に
 * 16px で描いていたころは実効 1.3px 前後で「同じ画面の本文の太さと揃う」と言えたが、
 * **40px の器に 20px で描くと実効 1.7px 前後**になる。**器・絵・線がそろって同じ倍率で
 * 伸びる**のは相似として正しく、要件が求めているのもそれである——**本文と揃うという
 * 理屈のほうが失効した**ので、そちらを書き換えてある。
 *
 * **一度 48px（1.5倍）まで大きくして、戻した**（2026-09-06・利用者の指定「大きすぎた。
 * 1.3倍くらいで良かった」）。実物の帯へ置くと、隣の ✕（32px）に対して 1.5倍は主張が
 * 強すぎた——**単体では大きすぎるか分からない**という形で踏んでいる。
 *
...
export function FilesToggle({ open, onToggle }: Props) {
  return (
    <Button
      ...
      onClick={onToggle}
    >
      {/* 寸法はボタン側が当てる（`icon-xl` は `size-5`＝20px を当てる） */}
      <svg
        aria-hidden
        viewBox="0 0 24 24"
        fill="none"
