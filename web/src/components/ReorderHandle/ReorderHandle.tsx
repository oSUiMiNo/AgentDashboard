 ...
 *
 * **区画（PJT 専用画面の横並び）は、WCAG 2.2 SC 2.5.7（ドラッグ以外の単一ポインタ手段）
 * に適合していない。** 掴み手で運ぶ以外の道が無い。
 *
 * **一覧のカードと枠は満たした**（2026-09-03・並べ替え設計§15-6）——帯の「前へ／後ろへ」と
 * Space で選ぶ道がある。**区画だけが保留**（設計§15-10）で、区画には「選ぶ」も帯も無いので
 * 同じ形が置けない。掴み手（この部品）のタップに意味を持たせる形が候補だが、決めていない。
 * 必要になったときに別のイシューとして起こす。
 *
