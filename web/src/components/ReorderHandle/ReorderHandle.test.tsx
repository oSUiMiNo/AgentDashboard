...
/**
 * **マウスは主ボタンでしか掴まない**（テスト計画フェーズ1）。
 *
 * `useGrip.ts` は `カードと枠を、中クリックで新しいタブに開く` で既に閉じたが、
 * **掴み手だけが取り残されていた**。中ドラッグでレールを横へ送るより先に塞ぐ。
 *
 * **束を分けてあるのは、壊し方を当てたときに落ちる範囲が名前で数えられるため。**
 * `掴む` に混ぜると、判定を消したときに落ちたのが新しい項目だけかを見分けにくい。
 */
describe('マウスは主ボタンでしか掴まない', () => {
  /**
   * ボタンと種類を指定して押す。
   *
   * **`pointerType` を必ず渡す。** 既存の束は渡しておらず jsdom では空文字で来るので、
   * 渡し忘れると判定の左半分（マウスに限る）が偽になり、直したあとも落ち続ける
   */
  function ボタンで押す(掴み手: HTMLElement, button: number, pointerType = 'mouse') {
    fireEvent.pointerDown(掴み手, {
      pointerId: 1,
      clientX: 100,
      clientY: 100,
      button,
  ...
  it('**中ボタンでは掴みが始まらない**——中ドラッグはレールの横送りに取ってある', () => {
    const { 掴み手, 記録 } = 置く()
    ボタンで押す(掴み手, 1)
    expect(記録).toEqual([])
  ...
  it('**指は今までどおり。** 条件をマウスに限っているか', () => {
    // **`button: 1` で書くのが要点。** `button: 0` だと判定を消しても通るので、
    // 条件の左半分を何も確かめないテストになる
    const { 掴み手, 記録 } = 置く()
    ボタンで押す(掴み手, 1, 'touch')
    expect(記録).toEqual(['grab'])
  })
