 ...
 *
 * **大きさは主従を担っていない**（細かい修正 項目11）。器ごとに 11px / 14.4px へ
 * 割ったので、**機械が差し込んだ吹き出し（11px）は活動の行（14px）より小さい**。
 * ここでいう「発言は主」が指しているのは**利用者とアシスタントの発言**であって、
 * 機械が差し込んだもの（フックの注入・完了通知・他セッションの連絡）は別である
 * ——あちらは**既に「無条件に畳む」決まりがあり、主として扱っていない**。
  ...
  const inner = (
    <>
      ...
      <div
        ...
        className={tone}
      >
        <div className={`prose-dashboard prose-body${fade ? ' body-fade-text' : ''}`}>
