 ...
 * **部品ごとに書くと、同じ断りが画面に何度も出る。** ここは `title` に最小限だけ添える。
 *
 * **【訂正 2026-09-13・フェーズ7】「画面の一箇所にまとめる」を「画面ごとに一箇所」へ改める。**
 * 近似の数字が**2つの画面に分かれている**——費用はここ（カード／セッションの区画）、
 * 使用上限と活動の記録は設定画面。**画面をまたいで一箇所にすると、片方に断りが付かない。**
 *
 * **禁じた理由は「同じ断りが画面に何度も出る」ことなので、画面をまたぐときには当たらない**
 * （同時に目に入らない）。設定画面側には `data-testid="approx-note"` で1つ置いた。
 *
 * **ここが `title` のままでよいのは、数字が1つしか無いため。** 設定画面は枠が2つあるので
 * `title` に頼らず文字で出している（ホバーは狭い窓とタッチで読めない）。
 */
 ...
 *
 * 印は**線で描く**（`DESIGN.md` §14.4 の絵文字禁止）。幹から1本が分かれて芽で終わる形。
 *
 * **§18.2 の下限を満たすよう寸法を取ってある**——主モチーフの外接は 24 のグリッドに対し
 * 横 77%・縦 79%（下限 70%）、要素は3つ（上限3）。**§18.4 の「塗り一色にして実寸で
 * 並べる」判定も実際に行った**：同じ列の3つ（拡大縮小・電源・終了）はどれも上下左右に
 * 対称な形で、**左寄りで非対称なのはこれだけ**なので 18〜22px でも混ざらない。
 */
