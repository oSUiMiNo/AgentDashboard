...
describe('SessionView の操作列は、区画の真上', () => {
  ...
  it('セッションに効くものは、両方の画面で操作列の中に居る', () => {
    ...
    */
    // **費用は材料が無いと1行も描かない**（`SessionCostLine` は `cost === null` で
    // `null` を返す）。`meta()` の既定が `cost: null` なので、**持たせないと
    // `session-cost` の行が必ず落ちる**——検査の不備ではなく材料不足である
    const 費用 = {
    ...
    for (const compact of [false, true]) {
      ...
      show(meta({ agent_connected: true, claude_session_id: 'sess-1', cost: 費用 }), compact)
      for (const 目印 of [
        'elapsed',
        'model-picker',
        ...
        // 言わない）が守られていないまま緑になる**
        'memo-toggle',
        // **費用もセッションに効くもの**（status 設計「この値は誰のものか」——費用は
        // セッションごと、使用上限はアカウントごと）。`compact` の分岐の外に置いて
        // あるので、1箇所で両方に出る。
        //
        // **ここへ登録する意味は、隣の2本と違う。** 「横並びでも出る」は*両方の画面に
        // 在るか*を、「2行目に居る」は*`data-row` の値*を見ている。**このリストだけが
        // `session-ops` の器の中に居るかを見ており、§33 の禁止「セッションに効く操作を
        // 画面の帯へ上げる」を守る唯一の検査である**——器の外へ出しても、隣の2本は
        // 緑のまま通る。
        //
        // **1件目がここへ「次に操作列へ何かを足す人は、まずこのリストを見ること」と
        // 残していた。このイシューがその「次の人」だった。**
        'session-cost',
      ]) {
