...
export function TerminalPane({ cardId }: Props) {
  ...
  useEffect(() => {
    ...
    /*
      **Shift ＋ 縦回しを、端末に食わせない**（`PJT画面の横スクロールをどこからでも効かせる`）。

      **端末のホイール購読は2つあり、止め方が違う。片方だけでは塞げない。**

      | 購読 | 何をする | 止め方 |
      |---|---|---|
      | 内部スクロール | 端末の箱を横へ動かす | 入口で `defaultPrevented` を見るので、**レール側の `preventDefault()` で止まる**（`lib/useRailPan.ts`） |
      | ここ | **スクロールバックがまだ無い間**、縦回しを ↑/↓ として claude へ送る | `defaultPrevented` を**見ない**ので、**この口でしか止まらない** |

      **レール側の手当てと重複していない。** 片方だけ見て「二重だ」と消すと、
      **起こした直後のセッションで、横へ送るたびに選択肢が上下に動く**——選択式の
      質問に答えている最中なら、答えが勝手に変わる。

      **返すのは `shiftKey` ＋ 縦のときだけ。** 修飾なしの縦まで止めると
      **端末の遡りが死ぬ**（要件は「通してよいのは横だけ」）。純横は
      `deltaY === 0` で向こうが自ら抜けるので、塞ぐ必要が無い。
    */
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      {/*
        **入れ物は「格子を覗く窓」である**（設計§3）。桁行を 120×40 に固定したので、
        ...
        | 指定 | 何のためか |
        |---|---|
        | `overflowX: auto` | 横にはみ出したぶんを読む道を**残す**ため。**PJT 画面ではホイールで読めない**——レールの横送りに横取りされる（`lib/useRailPan.ts`）。残るのは**指で払う・ドラッグ・スクロールバー**の3つ |
