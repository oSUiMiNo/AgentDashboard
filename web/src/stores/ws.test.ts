...
describe('起こし直しと購読', () => {
  ...
  */
  ...
  it('一覧から消えたカードの断りは持ち越さない', async () => {
    // 生死の台帳（`alive`）は消していたのに、断りの記録だけ残っていた。
    // **片方だけ消すと、仮にそのIDがもう一度現れたとき「生死は初めて見るのに
    // 断りだけ古い」という食い違った状態から始まる**——生死の側が出し直しを
    // 見送る一方で、断りの側が出し直してしまう
    await 開いて購読する()
    ...
    latest().deliver({ t: 'session_removed', card_id: CARD })
    latest().sent = []
