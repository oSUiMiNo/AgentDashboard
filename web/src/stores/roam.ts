...
/**
 * 退場を始めるまでの遅れ。**寿命の終わりへ寄せる。**
 *
 * **寿命ぶんのキーフレームを持たせない**——90秒ぶんの停留点を持たせると、そのあいだ
 * 基の形（`roam.css` の `[data-shape]`）が上書きされ続けて**3種の描き分けが消える**。
 * 遅らせて `animation-fill-mode: none` にすれば、**効くのは最後の 1.13秒だけ**になる。
 *
