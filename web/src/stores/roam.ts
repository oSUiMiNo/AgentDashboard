 ...
 * **70 ÷ 9.6 × 3 ≒ 22本**。**32 のまま据え置く**（フェーズ18）——上限に
 ...
 *
 * **90秒 → 70秒**（2026-08-28・要件15-8。利用者の指定）。
 *
 * **速さは「総道のり ÷ 寿命」で決まる**——寿命だけ縮めると速くなるので、
 * 幾何の側も一緒に動かした（`lib/roam.ts` の `ROAM_ROAM` 49→29。総道のり
 * 2917.4 → 1797.4px）。結果は **25.68px/秒**＝要件15-7「0.8倍」の 99.0%
 * （区間は整数なので割り切れない。設計§23-4）。
 */
export const ROAM_LIFE_MS = 70_000
