...
describe('機械の吹き出しの地（2026-09-06・鮮やかにした）', () => {
  // **行頭に錨を打つ。** `.speech-bubble.speech-bubble-machine {`（文字の大きさを
  // 上書きしている2クラスの規則）にも同じ字面が含まれるので、錨が無いと**先に
  // 書いてあるほうへ当たる**。ここで読みたいのは、地の色を持つ単クラスの規則である
...
describe('種別ごとの地（2026-09-06・利用者の指定）', () => {
  ...
  */
  ...
  it('サブエージェントの報告は、利用者が名指しした地になる', () => {
    // 数は字で書く。実装の定数から組み立てると、一緒に動いて通ってしまう
    //
    // **この色だけ、小さい字の床（4.5）を満たしていない**（地 4.10／コード地 3.51／
    // 名乗り 3.57）。利用者が「コントラストは若干妥協」と明示したうえで名指しした
    // `#D17A0F` と、その1つ前の `#9F5000` の**中点**である（`DESIGN.md` §45.6）。
    // **満たしていないことを承知で置いている**ので、「床を満たすように直す」と
    // 読んで勝手に暗くしないこと
    const 規則 = 取り出す('task_notification')
    expect(規則, "[data-origin='task_notification'] が見つからない").not.toBeNull()
    expect(規則![1]).toContain('#b86508')
  })
  ...
  it('どの地も1箇所でしか持たない', () => {
    expect((INDEX.match(/#b86508/g) ?? []).length).toBe(1)
