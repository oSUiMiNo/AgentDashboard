...
test('カードの操作は、マウスを乗せたときに4つ出る', async ({ page }) => {
  /*
  ...
  */
  ...
  /*
    **4つ**（メモ → 編集 → ゴミ箱 → 電源）。メモは 2026-09-13 に足したもので、
    **押しても何も変えないので先頭**に来る（上の「取り返しの付く順」の続き）。

    **上限は §15.3 ではなく §15.2 である。** ここには長く「3つまで（§15.3）。4つ目を
    足さない」と書いてあったが、**§15.3 の「群は2〜3個までにする」は群の個数**を
    言っている——直前の行が「間隔で群を切る」で、節の題も「区切りは線より間隔」である。

    **要素数と読むと §15.2 が意味を失う。** あちらは「5個を超えるなら畳む」と
    定めており、3個が上限ならその行は永久に効かない。

    `.tile-ops` は `display:flex` ＋ 一定の `gap` の**等間隔1列＝群は1個**なので、
    §15.3 には当たらない。**要素は5個まで置ける。いまは4つで、枠が1つ空いている。**

    **なお「5個に収まる」は必要条件であって十分条件ではない。** 一画面一アクセント
    （§9.3）と実サイズでの最終評価（§17.2・カード幅 466px）は別に見る。
  */
  await expect(群.locator('[data-testid]')).toHaveCount(4)
  await expect(器.getByTestId('memo-tile')).toBeVisible()
