...
test.beforeAll(() => {
  ...
  */
  ...
  fs.writeFileSync(path.join(docs, DEEP, "奥.md"), "# 奥\n", "utf8");
  fs.writeFileSync(
    path.join(docs, PLAN),
    "# 計画\n\n- [x] 済んだこと\n- [ ] まだのこと\n",
    "utf8",
  ...
  fs.writeFileSync(
    path.join(docs, PICTURE),
    Buffer.from(TINY_PNG_BASE64, "base64"),
  );
  // 拡張子は画像・中身は違う。**画面が壊れないこと**を見るための材料
  fs.writeFileSync(
    path.join(docs, BROKEN),
    "これは画像ではありません\n",
    "utf8",
  );
...
async function expectScrollable(box: Locator) {
  ...
  expect(
    (await scrollState(box)).scrollTop,
    "実際に遡れること",
  ).toBeGreaterThan(0);
}
...
test("新しい口が無いとき、古い方法が実物のブラウザで写す", async ({ page }) => {
  ...
  // ボタンは絵になったので（要件23）、答えは中の字で見る
  await expect(
    panel.getByTestId("folder-copy").first().getByTestId("folder-copy-state"),
  ).toHaveText("コピーしました");
...
test("どちらの口も無いとき、一覧とファイルの両方で値を選べる形が出る", async ({
  ...
  // ② 右クリック経由でも、同じ逃げ道が出ること（別の道を作っていない）
  await panel.getByTestId("folder-entry").first().click({ button: "right" });
  await page.getByTestId("folder-menu-copy-abs").click();
  await expect(panel.getByTestId("folder-copy-fallback")).toHaveText(
    `${PROJECT_DIR}/MyDocs`,
  );

  /*
    **ファイルの画面には、もう逃げ道が無い**（要件24 で「パスをコピー」を落とした）。
    写せない環境で詰まる形にはならない——写す道はサイドバーに残っており、
    そちらの逃げ道は上で出ている。
  */
  await panel.getByTestId("folder-entry").filter({ hasText: "MyDocs" }).click();
  await panel.getByTestId("folder-entry").filter({ hasText: PLAN }).click();
  const view = page.getByTestId("file-view");
  await expect(view.getByTestId("file-copy")).toHaveCount(0);
  await expect(view.getByTestId("file-copy-fallback")).toHaveCount(0);
});
 ...
 */
test("狭い画面でファイルを開いても、セッションの面が潰れない", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 780 });
  await openDashboard(page);
  ...
  const 中身 = await page.getByTestId("file-column").evaluate((el) => {
    const e =
      ...
      el.querySelector('[data-testid="file-body"]');
 ...
 * 片方でしか踏まない道ができる。
 */
test("ファイルでも、素の改行と `<br/>` が改行として出る", async ({
  page,
}, testInfo) => {
...
test("script は動くが、外へは1件も出ない", async ({ page }) => {
  ...
  const html = DANGEROUS_HTML.replaceAll(
    OUTSIDE_MARK,
    `${外.origin}/beacon.png`,
  );
  ...
  try {
    ...
    */
    ...
    const control = await page.evaluate(async (source) => {
      ...
      return (
        frame.contentDocument?.getElementById("印")?.textContent ?? "(読めず)"
      );
    }, html);
    expect(control, "隔離の外でなら、この材料の script は動く").toBe(
      "書き換えられた",
    );
    /*
...
test("箱の側から allow-scripts を外すと、script は動かなくなる", async ({
  page,
}) => {
  /*
...
test("生で返した .js は、script として実行できない", async ({ page }) => {
  ...
  */
  ...
  const 走った = await page.evaluate(async (src) => {
    ...
    return (
      (window as unknown as { __走った?: string }).__走った ?? "(走っていない)"
    );
  }, 宛先);
  ...
  const 裏取り = await page.evaluate(async () => {
    const blob = new Blob(['window.__裏 = "走った"'], {
...
test("ブラウザで開くと、新しいタブでも script が動く", async ({
  page,
  context,
}) => {
  /*
...
test("狭い窓でも、操作の列が横へはみ出さない", async ({ page }) => {
  ...
  */
  ...
  const 溢れ = await page.getByTestId("file-open-tab").evaluate((el) => {
...
test("壊れた画像でも画面は壊れない", async ({ page }) => {
  ...
  await expect(page.getByTestId("file-broken")).toContainText(
    "画像として読めません",
  );
  await expect(page.getByTestId("file-error")).toHaveCount(0);
  // 画面そのものは生きている（生テキストへの逃げ道は今までどおり押せる）
  await expect(page.getByTestId("file-open-tab")).toBeVisible();
});
...
test("狭い画面では、サイドバーは左端の帯で、裏が見えている", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 780 });
  await openDashboard(page);
  ...
  */
  ...
  expect(帯.width, "サイドバーは 20rem の帯（全画面にしない）").toBeCloseTo(
    320,
    0,
  );
  /*
...
test("狭い画面では、中身の列が画面いっぱいで、地の余白が残らない", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 780 });
  await openDashboard(page);
...
test("中身の列の上で横へ回すと、セッションのレールが動く", async ({ page }) => {
  // **列はレールの中にある**ので、ブラウザのスクロール連鎖（祖先を辿る）がそのまま
  // 届く。**手渡しは要らなくなり、消した**（`FileColumn` の `onWheelX`）。
  //
