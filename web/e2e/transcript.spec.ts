...
/*
  **この2本は緑である**（細かい修正 項目10）。**赤かった経緯を残す。**
  ...
  **(2) しかし症状そのものは実在した。** 下の `見えている畳める行()` で
  **画面に見えている行**に押し替えて測り直すと、位置は総高の増分ぶんそのまま動いた。

  | | 総高 | 位置 | 末尾からの距離 |
  |---|---|---|---|
  | 直す前・押す前 | 7489 | 7013 | 0 |
  | 直す前・押したあと | 10438 | **9962** | 0 |
  | **直したあと・押したあと** | 10438 | **7013** | **2949** |

  **直す前の増分 2949 は、総高の増分と1pxまで一致する。** 末尾に貼り付いたまま、
  伸びたぶんだけ下へ引かれていた——**錨（`anchorTo: 'end'`）が効いている姿**である。

  **人から見た症状はこうだった。** 押した行の「畳む」印は画面のほぼ同じ高さに留まる
  （624 → 633）が、それは印が本文の**下端**に浮いているためで、
  **開いた本文の頭は画面のはるか上へ流れていた。** 読もうとして押したのに、
  **その文章の末尾に着く。**

  **(3) 効かなかった手当てが1つある。** 案B（押す直前の位置を控えて戻す）は、
  2フレーム後に1回戻しても、12フレームのあいだ戻し続けても残った——`resizeItem` は
  `ResizeObserver` から何度も呼ばれ、**そのつどその瞬間の位置で条件を評価して書き換える**ため。
  **あとから戻す形は、戻す回数を増やしても結末が変わらない。**

  **(4) 効いた手当て**は `lib/tailAnchor.ts`——**押している間だけ `scrollEndThreshold` を
  負にして、錨が発火する条件そのものを偽にする。** 比べる相手（末尾からの距離）は
  必ず 0 以上なので、負にすれば必ず落ちる。**実装を外して測り直すと 9962 に戻る**ことを
  確かめてある（この2本が、その差を見張っている）。
...
test('末尾に居るとき、見えている「続きを読む」を押しても読んでいる位置が動かない', async ({
  page,
}) => {
  await loadFoldLines(page)
  await expect(foldableRow(page)).toBeVisible(届くまで)
...
test('末尾に居るとき、「畳む」を押しても末尾に留まる', async ({ page }) => {
