...
| どちらに出るか | もの | なぜ片側なのか |
|---|---|---|
| **単独画面だけ** | パスを押すと PJT 専用画面へ移る | 横並びのときは**既にその PJT の画面に居る** |
| **横並びだけ** | 「開く」を押すとセッション専用画面へ移る | 単独画面では**既にそのセッションの画面に居る** |

**これを「片方だけの実装」と読み違えないこと。** 理由は `SessionView.tsx` のその場のコメントに残してある。

**ただしこの表の2行は、`DESIGN.md` §39.5 で置き換わることが決まっている**（2026-09-02・利用者の指定）。行き来は**1つの切替ボタン**（拡大／縮小）にまとめ、**両方の画面の同じ場所**（電源ボタンのすぐ左）に置く。**「行き先がいま居る画面になる」という片側だけの理由は、切替ボタンには当たらない**——あれは常に別の画面へ行く。**上の2行は、置き換わるまでの記録として残してある。**
