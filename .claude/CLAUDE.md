...
したがって、イシューを1つ消化したら**3つを揃えるまでが1組**である。

| 何を | どうやって |
|---|---|
| **配る** | 版を上げて**タグだけ**を打つ（リリース本体は `dist` が作る。`gh release create` は使わない） |
| **サーバ** | AWS の `~/compose.yml` の `image:` を新しい版へ書き換え、`docker compose pull` → `up -d` |
| **手元** | 実機は `git pull` → `make build` → 起こし直す。**セッションホストも起こし直す**（サーバだけ新しくすると版がずれる） |
...
したがって実機の形はこうなる。**落とす判断だけ CLI に任せる。**

```
make build                          # 新しい実行ファイルを作る
