...
### 「更新する」ボタンを押すのは人
画面の「ダッシュボードの版」は、**保管庫（`<state_dir>/versions/`）に取ってきた版のあいだで切り替える**機能で、**選んでも即座には効かない**（次に起こしたときに効く＝予約）。すぐ効かせるには「いま入れ替える」を押す。

**押すのは人**（頼まれたときだけエージェントが押す）。走っているセッションが道連れになる操作なので、押す時機は人が決める。

構成によって、そもそも押す必要があるかが違う。

| 構成 | アップデートの効き方 |
|---|---|
| **ソースビルド**（実機・セッションホスト） | `make build` → 起こし直すだけで版が変わる。**押すボタンは無い**（予約が無ければ、実行ファイル自身の位置から起動する） |
| **配布バイナリ**（ワンライナーで入れた環境） | 「新しい版があります」→ 押して取ってくる → 選ぶ（予約）→ 次に起こしたときに効く |
| **docker compose**（AWS） | 版の切替そのものが画面に出ない。`image:` で決める |

**予約が残っていると、起こし直したときにそちらへ乗り換わる。** ソースビルドを新しくしたのに版が変わらないときは `<state_dir>/version-current` を見ること。
