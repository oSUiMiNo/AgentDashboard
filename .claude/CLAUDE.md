...
**ただし、いまリモート経路は無い。** **サーバは 2026-08-15 に消えた**（下記の段2）ので、**リモート経路（サーバ → セッションホスト）でしか再現しないものは踏めない**——外出先からの接続、セッションホストを別の機械に置いた構成、サーバを挟んだときの遅延や切断がこれにあたる。「配ったから確かめられる」と書いてある計画に当たったら、そこで止まることを先に見込むこと。

**ただし「スマホ＝踏めない」ではない。ここは 2026-09-05 に覆った。** かつてこの行には「スマホから触るものはリモート経路でしか再現しない」「スマホでの確認を要する受け入れは、サーバを立て直すまで踏めない」と書いてあったが、**誤りだった**——利用者が同じ Wi-Fi のスマホから `http://192.168.0.12:8787/` で繋ぎ、機能を実機で確認している。**LAN 直結はサーバを1バイトも経由しない。**

**スマホから開く道はこうである。** `bind_addr` を広げてあれば **`http://<Windows 側の LAN IP>:8787/`** で開ける（LAN の合言葉を聞かれる）。**番号は日に何度も変わる**ので、そのつど調べ直すこと——同じ日に `192.168.0.12` → `10.106.135.80` → `192.168.0.12` と2回動いた実測がある。**WSL の中で `hostname -I` が返す番号を使ってはいけない**——あれは横流し（portproxy）の受け先で、外からは届かない。**`192.168.` で始まるので本物に見える**のが厄介なところで、「172 で始まるものを除く」式の判定では素通りする。
...
**`0.1.79` から、タブは自分で読み直す**（`クローズ/版が切り替わったらタブを自動で読み直す`）。線が切れて繋ぎ直った瞬間に版を聞き直し、変わっていれば**押さなくても読み直る**——PC もスマホも同じで、どちらも触らなくてよい。`0.1.86` からは**一言出してから**読み直す（1.2秒。無言で入れ替わると故障に見えるため。**承認は求めない**）。**書きかけは消えない**（`lib/drafts.ts` が `localStorage` に置く）。

**「勝手には読み込み直さない——書きかけの指示が消える」は覆った。** かつてここにそう書いてあったが、**理由のほうが先に成り立たなくなっていた。**

**例外は添付を抱えたタブだけ。** 添付は読み直すと消えるので、そこだけバナーを出して人に任せる（**理由と件数が画面に出る**）。**抱えているのが画面外の入力欄でありうる**ことに注意——PJT 専用画面はセッション全数の入力欄を仮想化なしに描くので、横スクロールの外にある1つが読み直しを止める。実際にそれで不発になり、原因の特定に2日かかった。

**判断はログに残る**（`kind=version_reload`）。読み直したのか、見送ったのか（と件数）が端末ごとに出るので、**「効かなかった」と言われたらまずこれを読む。**

**道具を入れた直後の1回だけは、手で押すことになる。** 自動読み直しは**その機能を含む画面が読み込まれて初めて効く**ので、機能が入る前に開かれていたタブには読み直すコードが無い。**新しい画面の振る舞いを実機で確かめられるのは、配った次の切替から**である。
...
**「サーバ」の段は書かない。** かつては3つだったが、**サーバは無くなった**（上の段2）。書くと、踏めない項目が未消化のまま毎回積み上がる。**リモート経路でしか再現しない項目は、立てられないことを理由に【保留】として分けておくこと**——【要人間】に混ぜると、人が居ても踏めないものと、人さえ居れば踏めるものが見分けられなくなる。

**【保留】は狭い。「スマホ＝【保留】」と機械的に振らないこと**（2026-09-05 に線を引き直した。上の節）。判定の問いは1つである。

> **これは、同じ Wi-Fi に居るスマホで再現するか。再現するなら【要人間】。サーバを挟まないと現れないものだけが【保留】。**

| 種類 | 例 | 分類 |
|---|---|---|
| **画面の中で完結する**（描画・入力・CSS・CSP） | 狭い窓での折り返し、タッチの効き、吹き出しが押せるか | **【要人間】。LAN で踏める** |
| **セッションホストを別の機械に置かないと再現しない** | 別の PC のセッションを外出先から操作する | **【保留】。サーバが立つまで踏めない** |

**既に【保留】と振ってあるものは、次にそのイシューへ触るときに判定し直す**（一斉に洗い直す必要はない）。**線を狭めたのは 2026-09-05 なので、それ以前に振られた【保留】は広すぎる疑いがある**——実例は `クローズ/イシューグループ_2026-0903-0422/クローズレポート.md` §7-1 で、2件とも【要人間】が正しかった。
...
| `web/src/lib/composerBusy.ts` | **版が切り替わったとき、そのタブが自分で読み直してよいか**の唯一の門（`App.tsx` の `ServerChangedBanner` が読む）。抱えているのは**画面外の入力欄でありうる**——PJT 専用画面はセッション全数の入力欄を仮想化なしに描く。**鍵はカードではなく札**（`Set<object>`）で、登録ごとに別物なので解除の本人確認が要らない |
