...
製品に効くパス（`.claude/`・`MyDocs/`・`docs/`・ルートの `*.md` 以外）を触ったコミットが0件なら配らない（文書・テスト・CI だけの変更は次の版に相乗りさせる）。定期のときは静かに終わり、明示（`--now`）のときはそう報告する。
...
門0（汚れ）：`bump` の前に4ファイルが未コミットでないことを確かめる。汚れていたら止まる（他セッションの行を巻き込むため）。
...
**止まる条件**：`claim` の終了コードが1（他セッションが `__版上げ__` を握っている）または3（札の仕組みが無い、または複数あって決められない）。どちらでも版上げは止まる。門2が赤
...
**止まる条件**：`claim` の終了コードが1（他セッションが `__重いテスト__` を握っている）または3（札の仕組みが無い、または複数あって決められない）。どちらでも版上げは止まる。門4が赤
